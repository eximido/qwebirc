...
qwebirc.ui.themes.Default = {
  "PREFIX": ["$C4==$O "],
  "SIGNON": ["Мы в сети!", true],
  "CONNECTING": ["Подключение к IRC-серверу...", true],
  "CONNECT": ["Авторизация...", true],
  "CONNECTED": ["Connected and logged in -- ready to go!", true],
  "RAW": ["$m", true],
  "DISCONNECT": ["Отключение от сервера: $m", true],
  "ERROR": ["ERROR: $m", true],
  "SERVERNOTICE": ["$m", true],
  "DISCONNECTWG": ["Отключение от вебгейта: $m", true],
  "JOIN": ["${$N$} [$h] заходит на канал $c", true],
  "OURJOIN": ["${$N$} [$h] заходит на канал $c", true],
  "PART": ["${$N$} [$h] покидает канал $c [$m]", true],
  "KICK": ["${$v$} was kicked from $c by ${$N$} [$m]", true],
  "MODE": ["mode/$c [$m] by ${$N$}", true],
  "QUIT": ["${$N$} [$h] покидает чат [$m]", true],
  "NICK": ["${$n$} меняет ник на ${$[$w$]$}", true],
  "TOPIC": ["${$N$} меняет заголовок $c на: $m", true],
  "UMODE": ["Режим: $m", true],
  "INVITE": ["$N приглашает зайти на $c", true],
  "HILIGHT": ["$C4"],
  "HILIGHTEND": ["$O"],
  "CHANMSG": ["<$C14$@$O${$($N$)$}> $m"],
  "PRIVMSG": ["<$($N$)> $m"],
  ...
  "WHOISCHARSET": [" charset  : $m", true],
  "WHOISREALHOST": [" realhost : $m", true],
  "WHOISENCRYPTED": ["          : $m", true],
  "WHOISEND": ["End of WHOIS", true],
  "WHOISENDW": ["End of WHOWAS", true],
